...
import { ActiveMessagesPage } from '@/components/dashboard/active-messages-page' // <-- NOVO IMPORT
...
export function Dashboard() {
      ...
      items.forEach((item: any) => {
        ...
        if (typeof action === 'string' && action.startsWith('{')) {
          try {
            ...
            action = parsed.resource?.team || parsed.type || parsed.name || 'Default'
          } catch {
            action = 'Unknown'
          }
        }
  ...
  const activeChartFilters = useMemo(() => {
    ...
    const fields = [selectedChart.xField, 'category', 'action', 'state_name']
    ...
    return fields
      ...
      .map((field) => {
        ...
        return {
          field,
          label: field === 'state_name' ? 'Estado' : field === 'action' ? 'Ação' : field === 'category' ? 'Categoria' : field,
          options,
        }
      })
        ...
        <div className="flex-1 flex min-h-0 overflow-hidden">
          {!currentBranch ? (
            ...
            <>
              ...
              {/* Oculta os filtros nas páginas 3 e 4, já que elas têm componentes e lógicas próprias */}
              {currentPageId !== 'page-3' && currentPageId !== 'page-4' && (
                <FilterPanel
